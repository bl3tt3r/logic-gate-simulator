//! Porte logique NOR — inverse de OR : sortie active uniquement si aucune entrée ne l'est.
...
/// Plugin Bevy pour la porte NOR.
pub struct Nor;

const PICTURE: &str = "gates/NOR.png";

/// Identifiant de l'item NOR dans le menu de sélection.
#[derive(Resource, Deref)]
pub struct NorItemId(pub usize);

/// Composant marquant une entité comme porte NOR.
#[derive(Component)]
pub struct NorGate;
...
impl Plugin for Nor {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, add_menu_item)
            ...
            .add_systems(Update, compute_nor_gate.in_set(GateSet::Compute));
    }
}

/// Calcule la sortie NOR : active uniquement si aucune entrée n'est active.
fn compute_nor_gate(
    gates: Query<&Children, With<NorGate>>,
...
fn add_menu_item(mut commands: Commands, mut menu_items: ResMut<MenuItems>) {
    let id = menu_items.insert("NOR", PICTURE);
    commands.insert_resource(NorItemId(id));
}

fn manage_preview(
    mut commands: Commands,
    selected: Res<SelectedItem>,
    nor_id: Res<NorItemId>,
    ...
    match selected.0 {
        Some(id) if id == **nor_id => spawn_preview(&mut commands, &asset_server, PICTURE),
