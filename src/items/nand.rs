//! Porte logique NAND — inverse de AND : sortie active sauf si toutes les entrées le sont.
...
/// Plugin Bevy pour la porte NAND.
pub struct Nand;

const PICTURE: &str = "gates/NAND.png";

/// Identifiant de l'item NAND dans le menu de sélection.
#[derive(Resource, Deref)]
pub struct NandItemId(pub usize);

/// Composant marquant une entité comme porte NAND.
#[derive(Component)]
pub struct NandGate;

impl Plugin for Nand {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, add_menu_item)
            .add_systems(Update, (manage_preview, on_drop))
            .add_systems(Update, compute_nand_gate.in_set(GateSet::Compute));
    }
}

/// Calcule la sortie NAND : active sauf si toutes les entrées sont actives.
fn compute_nand_gate(
    gates: Query<&Children, With<NandGate>>,
...
fn add_menu_item(mut commands: Commands, mut menu_items: ResMut<MenuItems>) {
    let id = menu_items.insert("NAND", PICTURE);
    commands.insert_resource(NandItemId(id));
}

fn manage_preview(
    mut commands: Commands,
    selected: Res<SelectedItem>,
    nand_id: Res<NandItemId>,
    ...
    match selected.0 {
        Some(id) if id == **nand_id => spawn_preview(&mut commands, &asset_server, PICTURE),
